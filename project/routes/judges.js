  ...
  try {
    if (req.session.user_id == undefined) {
      ...
      const records = await DButils.execQuery(
        `SELECT * FROM dbo.sadna_judges WHERE user_id = '${req.session.user_id}' and league = '${req.body.leagueID}' and season = '${req.body.season}'`
      );
      if (records.length >= 1) {
        throw {
          status: 409,
      ...
      await DButils.execQuery(
        `INSERT INTO dbo.sadna_judges (user_id, league, season) VALUES ('${req.session.user_id}', '${req.body.leagueID}', '${req.body.season}')`
      );
