...
router.post("/addNewGame", async (req, res, next) => {
  try {
    ...
    if (
      gameDB.find(
        (x) =>
          x.GameDate === date &&
          x.Session === session &&
          x.HomeTeamName == home_team_name &&
          x.AwayTeamName == away_team_name
      )
    )
      throw {
        status: 409,
        message: "The teams can't play on that date, it's taken",
      };
    ...
    let isJudge2 = false;
    let isJudge3 = false;
    const judgeDB = await DButils.execQuery(
    ...
    await DButils.execQuery(
      `INSERT INTO dbo.sadna_games (Season, League, HomeTeamName, AwayTeamName,GameDate, Location,MainJudge,Judge1,Judge2,Judge3) VALUES 
      ('${season}','${league}','${home_team_name}','${away_team_name}','${date}', '${location}','${mainJudge}','${judge1}','${judge2}','${judge3}')`
    );
