...
afterAll(async () =>{
    ...
    await user_handler.deleteUserForTest("referee_addGames");
...
test('successfull add games', async () => {
    ...
    await use_cases.RejisterJudgeUC("referee_addGames", "test_league1", "test_season1");
...
test('only 1 referees for season-league', async () => {
    ...
    //login with RoFA
    await use_cases.LoginUC("rofa_addGames", "password123");
    await use_cases.RejisterJudgeUC("referee_addGames", "test_league2", "test_season1")
...
test('unauthorized operation - logged in with non RoFA user', async () => {
    expect.assertions(2);
    await use_cases.LoginUC("referee_addGames", "password123");
    const error = (await use_cases.AddGamesUC("test_league1", "test_season1")).response;
    expect(error.status).toEqual(401);
    expect(error.data).toEqual("Only RoFA can add new games");
});
...
test('games already created', async () => {
    expect.assertions(2);
    await use_cases.LoginUC("rofa_addGames", "password123");
    await use_cases.RejisterJudgeUC("referee_addGames", "test_league2", "test_season2");
